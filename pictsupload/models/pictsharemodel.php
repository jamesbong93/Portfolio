...
class PictshareModel extends Model
{
	...
	function renderUploadForm()
	{
		...
		return '
		<div class="clear"></div>
		<strong>文件最大上传容量：'.$maxfilesize.' MB</strong><br>
		<strong>*您可以一次性的选择多个文件上传</strong>
		<br><br>
		<FORM id="form" enctype="multipart/form-data" method="post">
		<div id="formular">
			'.$upload_code_form.'
			<input class="btn btn-primary btn-outline choose_btn btn-lg" type="file" name="pic[]" multiple>
			<div class="clear"></div>
			<br>
		</div>
			<button class="btn btn-primary btn-outline btn-lg"  type="submit" id="submit" name="submit" value="dd" onClick="setTimeout(function(){document.getElementById(\'submit\').disabled = \'disabled\';}, 1);$(\'#movingBallG\').fadeIn()"><span class="glyphicon glyphicon-upload" aria-hidden="true"></span>上传图片</button>
	...
	function ProcessUploads()
	{
		...
		foreach ($_FILES["pic"]["error"] as $key => $error)
		{
			...
			if ($error == UPLOAD_ERR_OK)
			{
				...
				if($data['status']=='OK')
				{
					$picImageLink = DOMAINPATH.PATH.$data['hash'];
					if($data['type']=='mp4')
						$o.= '<h2>'.$this->translate(4).' '.++$i.'</h2><a target="_blank" href="'.DOMAINPATH.PATH.$data['hash'].'">'.$data['hash'].'</a><br>
							<p>'.DOMAINPATH.PATH.$data['hash'].'</p><br/>';
					else
						$o.='<div>';
						$o.= '<h2>'.$this->translate(4).' '.++$i.'</h2><a target="_blank" href='.$picImageLink.'><img class="preview_pic" src="'.DOMAINPATH.PATH.'300/'.$data['hash'].'" /></a><br><br>';
						$o.='<strong></strong><div ><label>图片地址: </label><input  class="form-control" onClick="this.setSelectionRange(0, this.value.length)" value="'.$picImageLink.'" /></div><br>&nbsp&nbsp';
						$o.='</div>';
