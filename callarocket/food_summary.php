...
/*
	...
	$sql_insert = "INSERT INTO confirmedrequest(userName, contactNumber, userAddress, category, storeList, timeFrame, requestBody, smsCode, promoCode, order_way ) VALUES
				 ('$name', '$contact', '$address', '$category', '$restaurant', '$time_slot', '$detail','$smsCode', '$promoCode', '$order_way')";

	$conn->query($sql);
				

$sql_select = "SELECT * FROM confirmedrequest,userrequests WHERE confirmedrequest.id = ( SELECT max(id) FROM confirmedrequest )AND userrequests.id = ( SELECT max(id) FROM userrequests ) AND confirmedrequest.smsCode = userrequests.smsCode";



$sql_select = "SELECT * FROM confirmedrequest WHERE id = ( SELECT max(id) FROM confirmedrequest )";


$result = $conn->query($sql_select);

while($row = $result->fetch_assoc()){
$sms_code = $row["smsCode"];	
$name = $row["userName"];
$contact = $row["contactNumber"];
$address = $row["userAddress"];
$time_slot = $row["timeFrame"];
$restaurant = $row["storeList"];
$detail = mysqli_real_escape_string($conn,$row["requestBody"]);
      ...
      <form name = "myform" class="form-horizontal" method = "post"  >
		 ...
		 <!-- ID input -->
          <div class="form-group">
			...
			<div class="col-sm-10">
			<label class="col-sm-10 "><?php echo $_SESSION["sms"];?> </label>
            </div>
          </div>
