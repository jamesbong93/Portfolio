		...
		<nav class="navbar navbar-inverse navbar-fixed-top" role="navigation">
			...
			<div class="collapse navbar-collapse navbar-ex1-collapse">
                ...
                <ul class="nav navbar-nav side-nav">
					...
					<li  class="active">
                        <a href="today_order.php?admin_email=<?php echo $admin_email;?>"><i class="fa fa-fw fa-desktop"></i>  Today orders <span class="badge alert-info"> <?php echo $today_counts;?> </span> </a>
                    </li>
					<li>
	 ...
	 <div class="container-fluid">
				<div class="row">
                    <div class="col-lg-12">
                        ...
                        <ol class="breadcrumb">
                            <li class="active">
                                <i class="fa fa-dashboard"></i> Today orders
                            </li>
                        </ol>
                    </div>
                </div>
				...
				$query_today = "SELECT * FROM purchase_history,user_login,books
							where purchase_history.User_ID = user_login.ID 
							and purchase_history.Book_ID = books.ISBN
							and purchase_history.Purchase_date LIKE '%". date("Y-m-d") . "%' ORDER BY Purchase_ID DESC";

				$query_read = "UPDATE purchase_history SET Read_check = 'yes' WHERE Read_check = 'no'";
				 $conn->query($query_read);	
				...
				while($row = $result->fetch_assoc()) {
					...
					echo "<tr>"; 
						echo "<td>".$row["Purchase_ID"];
						echo "<td>".$row["User_ID"];
						echo "<td>".$row["Book_ID"];
						echo "<td>".$row["username"];
						echo "<td>".$row["Name"];
						echo "<td>".$row["Phone_num"];
						echo "<td>".$row["SMS_code"];
						echo "<td>".$row["Payment_method"];
						echo "<td>".$row["Purchase_Date"];
					echo "</tr>"; 
