(function($){
	var menu_flip_speed = 200,
		...
		$cloned_nav,
		et_slider,
		$multi_media_bar = $('#et-multi-media #media-slides');
...
//+ '<span class="menu_slide">' + li_text + '</span>' 
		...
		$multi_media_bar.find('.thumb a').hover(function(){
			...
			jQuery(this).parent('.thumb').find('.media-description').stop(true,true).css({'display':'block','opacity':0}).animate({opacity:1, bottom:'113px'},400);
		} ,function(){
			jQuery(this).find('img').stop(true,true).fadeTo(400,1).end().find('span').fadeTo(400,0);
			jQuery(this).parent('.thumb').find('.media-description').stop(true,true).animate({opacity:0, bottom:'123px'},400,function(){
			jQuery(this).css({'display': 'none'});
			});
		});
		
		//$(".ls_dar_ser_wrapper > ul >li:first-child").hide();
	});
